...
public class Vector : MonoBehaviour {
    ...
    void Update() {
        ...
        output *= 0.001;
        float magnitude = (float)output.Magnitude;
        sprite.color += new Color(0.01f * magnitude, -0.01f * magnitude, -0.01f * magnitude);
        if (magnitude > MAX_MAGNITUDE) {
            transform.localScale = new UnityEngine.Vector3(MAX_MAGNITUDE, MAX_MAGNITUDE / 2, 1f);
        }
        else {
            transform.localScale = new UnityEngine.Vector3(magnitude, magnitude * 0.5f, 1f);
        }
